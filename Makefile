...
CAMEL_VERSIONS?=3.20 3.21 4.0 4.1 4.2 4.3 4.4
...
$(CAMEL_VERSIONS): dest-dir
	...
	$(foreach host,$(TEST_HOSTS),scp camel-load-testers/camel-load-tester-consumers/camel-load-tester-consumer-kafka/target/camel-load-tester-consumer-kafka-$@*.jar $(host):$(TESTER_DIR); )
	$(foreach host,$(TEST_HOSTS),scp camel-load-testers/camel-load-tester-consumers/camel-load-tester-consumer-netty-http/target/camel-load-tester-consumer-netty-http-$@*.jar $(host):$(TESTER_DIR); )
	$(foreach host,$(TEST_HOSTS),scp camel-load-testers/camel-load-tester-consumers/camel-load-tester-consumer-platform-http-vertx/target/camel-load-tester-consumer-platform-http-vertx-$@*.jar $(host):$(TESTER_DIR); )
