...
public class FilterDataSonnetPositive extends ThreadedProducerTemplate {
    private static final String POSITIVE = "positive";

    private static final Logger LOG = LoggerFactory.getLogger(FilterDataSonnetPositive.class);
    private final LongAdder longAdder;

    public FilterDataSonnetPositive() {
    ...
    protected void produceMessages(int numMessages, ProducerTemplate producerTemplate, Endpoint endpoint) {
        ...
        for (int i = 0; i < numMessages; i++) {
            producerTemplate.sendBody(endpoint, POSITIVE);
        }
    }
    ...
    public void configure() {
        ...
        from("disruptor:test")
                .filter(datasonnet("ds.toString(body) == 'positive'"))
